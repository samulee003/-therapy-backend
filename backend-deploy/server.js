...
async function initializeDatabase() {
  ...
  try {
    ...
    if (!settingsRow) {
      ...
      const defaultPassword = '[REDACTED]'; // 考慮使用更安全的預設值或強制修改
      const hashedPassword = await bcrypt.hash(defaultPassword, saltRounds);
      const defaultSlots = JSON.stringify(['09:00', '10:00', '11:00', '14:00', '15:00', '16:00']);
      await runDb('INSERT INTO settings (id, adminPassword, defaultTimeSlots) VALUES (1, ?, ?)', [
        hashedPassword,
        defaultSlots,
      ]);
      console.log('預設設置已插入。');
    ...
    if (usersRow && usersRow.count === 0) {
      ...
      const defaultUserPassword = '[REDACTED]';
      ...
      await runDb(
        ...
        ['doctor@example.com', hashedUserPassword, 'Dr. Demo', 'doctor', '12345678']
      );
      await runDb(
        'INSERT INTO users (username, password, name, role, phone) VALUES (?, ?, ?, ?, ?)',
        ['patient@example.com', hashedUserPassword, 'Patient Demo', 'patient', '87654321']
      );
      console.log('預設用戶已插入。');
    } else if (usersRow) {
...
app.post('/api/register', async (req, res) => {
  ...
  if (!username || !password || !name || !role || !phone) {
    return res
      ...
      .json({
        success: false,
        message: '缺少必要的註冊欄位 (username, password, name, role, phone)。',
      });
  }
...
app.get('/api/me', isAuthenticated, async (req, res) => {
  try {
    ...
    const user = await getDb('SELECT id, username, name, role, phone FROM users WHERE id = ?', [
      req.user.userId,
    ]);
    if (!user) {
      ...
      return res
        .status(404)
        .json({ success: false, message: '用戶不存在或認證信息無效，請重新登入。' });
    }
...
app.get('/api/settings', isAuthenticated, async (req, res) => {
  ...
  try {
    const settings = await getDb(
      'SELECT doctorName, clinicName, notificationEmail, defaultTimeSlots FROM settings WHERE id = 1'
    );
    if (!settings) {
      // 如果沒有設置，返回一個預設結構或錯誤
    ...
    if ((userRole === 'doctor' && !doctorId) || doctorId) {
      ...
      scheduleRows.forEach(row => {
        ...
        try {
          ...
          console.error(`解析 ${row.date} 的 availableSlots 失敗:`, e);
        }
        try {
          bookedSlots = JSON.parse(row.bookedSlots || '{}');
        } catch (e) {
          console.error(`解析 ${row.date} 的 bookedSlots 失敗:`, e);
        }
...
app.post('/api/schedule', isAuthenticated, isDoctor, async (req, res) => {
  ...
  if (
    ...
    !availableSlots.every(slot => /^([01]\d|2[0-3]):([0-5]\d)$/.test(slot))
  ) {
    return res
      .status(400)
      .json({ success: false, message: 'availableSlots 必須是 HH:MM 格式的時間數組。' });
  }
  // 去重並排序
  ...
  try {
    // 獲取當前已預約的時段，以防覆蓋
    const existingSchedule = await getDb(
      'SELECT bookedSlots FROM schedule WHERE date = ? AND doctorId = ?',
      [date, doctorId]
    );
    ...
    console.log(
      `醫生 ID ${doctorId} 正在${operationType}: ${date}, 時段數: ${availableSlots.length}`
    );
    ...
    res.json({
      success: true,
      message: `日期 ${date} 的排班已保存。${isRestDay ? '已設為休假日。' : ''}`,
    });
  } catch (error) {
...
app.post('/api/book', isAuthenticated, isPatient, async (req, res) => {
  ...
  if (!date || !time || !patientName || !patientEmail) {
    return res
      .status(400)
      .json({ success: false, message: '缺少必要的預約信息 (日期, 時間, 患者姓名, 患者郵箱)。' });
  }
  if (!/^\d{4}-\d{2}-\d{2}$/.test(date)) {
    ...
    await db.serialize(async () => {
      ...
      try {
        ...
        const schedule = await getDb(
          'SELECT availableSlots, bookedSlots FROM schedule WHERE date = ? AND doctorId = ?',
          [date, doctorId]
        );
        if (!schedule) {
        ...
        let bookedSlots = {};
        try {
          availableSlots = JSON.parse(schedule.availableSlots || '[]');
        } catch (e) {}
        try {
          bookedSlots = JSON.parse(schedule.bookedSlots || '{}');
        } catch (e) {}
        ...
        const updateScheduleSql =
          'UPDATE schedule SET availableSlots = ?, bookedSlots = ? WHERE date = ? AND doctorId = ?';
        await runDb(updateScheduleSql, [
          JSON.stringify(updatedAvailableSlots),
        ...
        res
          .status(201)
          .json({ success: true, message: '預約成功！', appointmentId: newAppointmentId });
      } catch (innerError) {
        // 如果事務內部出錯，回滾
        ...
        if (
          ...
          innerError.message.includes(
            'UNIQUE constraint failed: appointments.date, appointments.time, appointments.doctorId'
          )
        ) {
          res
            .status(409)
  ...
  try {
    await db.serialize(async () => {
      ...
      try {
        ...
        const schedule = await getDb(
          'SELECT availableSlots, bookedSlots FROM schedule WHERE date = ?',
          [appointment.date]
        );
        if (schedule) {
          let availableSlots = [];
          ...
          if (bookedSlots[appointment.time]) {
            ...
            console.warn(
              `警告：在排班表中未找到預約 ${appointmentId} (${appointment.date} ${appointment.time}) 的 bookedSlot 記錄。`
            );
          }
        } else {
          console.warn(
            `警告：未找到預約 ${appointmentId} 對應日期 ${appointment.date} 的排班記錄。`
          );
        }
